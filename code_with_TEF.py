...
file_path = 'E:\crowd.scene.analasys\h264\crowd002.avi'
...
w1=int(w/16)
br=0
se  = np.arange(w1*h1).reshape(h1,w1,1)
for i in range(h1):
    for j in range(w1):
        se[i][j] =0
...
fhand = open('E:\crowd.scene.analasys\h264\crowd002.txt')
for line in fhand:
...
fhand1 = open('E:\crowd.scene.analasys\h264\crowd002.txt','r')
...
ser  = np.arange(w*h).reshape(h,w,1)
for i in range(h):
    for j in range(w):
        ser[i][j] =0

w8  = np.arange(w*h).reshape(h,w,1)
for i in range(h):
    for j in range(w):
        w8[i][j] =0
w2  = np.arange(w*h).reshape(h,w,1)
for i in range(h):
    for j in range(w):
        w2[i][j] =0
bx  = np.arange(w*h).reshape(h,w,1)
for i in range(h):
    for j in range(w):
        bx[i][j] =0

by  = np.arange(w*h).reshape(h,w,1)
for i in range(h):
    for j in range(w):
        by[i][j] =0

ex1  = np.arange(w*h).reshape(h,w)
for i in range(h):
    for j in range(w):
        ex1[i][j] =0

ey1  = np.arange(w*h).reshape(h,w)
for i in range(h):
    for j in range(w):
        ey1[i][j] =0

ex2  = np.arange(w1*h1).reshape(h1,w1,1)
for i in range(h1):
    for j in range(w1):
        ex2[i][j] =0

ey2  = np.arange(w*h).reshape(h,w,1)
for i in range(h):
    for j in range(w):
        ey2[i][j] =0
...
for i in range(h):
    for j in range(w):
        if ser[i][j]>ft:
             w8[i][j]=int(w8[i][j]/ser[i][j])
             w2[i][j]=int(w2[i][j]/ser[i][j])
...
fig, ax = plt.subplots()
ax.quiver(l1, l2, z6, z7, scale=100,color='y')
ax.axis([0, w-1,h-1, 0])
...
for i in range(h):
    for j in range(w):
        if bx[i][j] != 0 or by[i][j] != 0   and(i):
            for i1 in range(h):
                for j1 in range(w):
                 if bx[i1][j1] != 0 or by[i1][j1] != 0:
                    if  i1!=i    or    j1!=j  :
                        v1 = bx[i][j] * bx[i][j] + by[i][j] * by[i][j]
                        v2 = bx[i1][j1] * bx[i1][j1] + by[i1][j1] * by[i1][j1]
                        v3 = math.sqrt(v1)
                        v4 = math.sqrt(v2)
                        v5 = v3 * v4
                        v6 = bx[i][j] * bx[i1][j1] + by[i][j] * by[i1][j1]
                        v7 = v6 / v5
                        if v7>=0.7:
                            dist=((i1-i)*(i1-i)+(j1-j)*(j1-j))/16
                            dist1=((j-j1)*bx[i1][j1])/16
                            dist2=((i-i1)*by[i1][j1])/16
                            dist3 = np.exp(-0.2*dist)
                            dist4 = np.exp(-0.8*abs(dist1+dist2))
                            #print(dist,dist3,dist4)
                            ex1[i][j]   =  ex1[i][j]    +  10*(bx[i1][j1]*dist3*dist4)
                            ey1[i][j] = ey1[i][j] +     10*(by[i1][j1] *dist3*dist4)

            #if   ex1[i][j] != 0 or ey1[i][j] != 0:
                #print(ex1[i][j],ey1[i][j])
                #ex1[i][j] = int(ex1[i][j])
                #ey1[i][j] = int(ey1[i][j] )
                #print(ex1[i][j], ey1[i][j])
...
#for i in range(h):
    #for j in range(w):
             #ex1[i][j]=int(ex1[i][j]/ser[i][j])
             #ey1[i][j]=int(ey1[i][j]/ser[i][j])

             #if ex1[i1][j1] != 0 or ey1[i1][j1] != 0:
                #print(ex1[i][j],ey1[i][j],'dd')

for i in range(h):
    for j in range(w):
        if ex1[i][j]!=0 or ey1[i][j]!=0 :

                    l11.insert(oreowex,j)
                    l22.insert(oreowex,i)
                    z66.insert(oreowex,ex1[i][j])
                    z77.insert(oreowex,ey1[i][j])
                    oreowex = oreowex + 1


fig, ax = plt.subplots()
ax.quiver(l11, l22, z66, z77, scale=4000,color='y')
ax.axis([0, w-1,h-1, 0])
fig.subplots_adjust(0,0,1,1,0.2,0.2)
plt.savefig('E:\crowd.scene.analasys\h264\pew12\image32'+str(p)+'.png')
plt.show()
...
w891= np.arange((oreowex)*4).reshape((oreowex,4))
w892= np.arange((oreowex)*2).reshape((oreowex,2))
for i in range(oreowex):
...
for i in range(oreowex):
    ...
    if labels[i] == 2   and sew[w891[i][1]][w891[i][0]]==0:
        r1 = 0
        n1=w891[i][1]
        n2=w891[i][0]
        while r1 < 16:
